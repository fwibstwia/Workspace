...
let get_dp_form_constant = foreign "getDPFormConstant"(ppl_manager @-> float @-> returning dp_form)
let get_dp_form_variable = foreign "getDPFormVariable"(ppl_manager @-> int @-> returning dp_form)
let get_dp_form_plus = foreign "getDPFormPlus"(ppl_manager @-> dp_form @-> dp_form @-> returning dp_form)
let get_dp_form_minus = foreign "getDPFormMinus"(ppl_manager @-> dp_form @-> dp_form @-> returning dp_form)
let get_dp_form_times = foreign "getDPFormTimes"(ppl_manager @-> dp_form @-> dp_form @-> returning dp_form)				
