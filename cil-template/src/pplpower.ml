...
let copy_pm = foreign "copy"(ppl_manager @-> returning ppl_manager)
...
let ppl_refine_bad_state = foreign "refineBadState"(ppl_manager @-> int @-> dp_form @-> returning bool)
...
let ppl_eval_equal_constraint = foreign "evalEqualConstraint"(ppl_manager @-> dp_form @-> dp_form @-> returning void)
