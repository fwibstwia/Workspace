...
transformerSteps =
  [
    ...
    (replaceUnaryOps, "replaceUnaryOps"),
    (transIfWithEq, "transIfWithEq"),
...
replaceUnaryOps :: Data a => a -> TransformerM a
replaceUnaryOps tu = return $ everywhere (mkT replaceUnaryOps') tu
  where
    replaceUnaryOps' :: CExpr -> CExpr
    replaceUnaryOps' (CUnary CPlusOp expr node) =
        CBinary CAddOp (CConst (CFloatConst (CFloat "0.0") un)) expr node
    replaceUnaryOps' (CUnary CMinOp expr node) = 
        CBinary CSubOp (CConst (CFloatConst (CFloat "0.0") un)) expr node
    replaceUnaryOps' e = e

transIfWithEq :: Data a => a -> TransformerM a
transIfWithEq tu = return $ everywhere (mkT transIfWithEq') tu
  where 
    transIfWithEq' :: CStat -> CStat
    transIfWithEq' (CIf cond tS eS node) = CIf (replaceNE cond) tS eS node
    transIfWithEq' s = s

replaceNE :: CExpr -> CExpr
replaceNE (CBinary CLeOp exprL exprR node) = 
    CBinary CEqOp exprL exprR node
replaceNE (CBinary CGrOp exprL exprR node) = 
    CBinary CEqOp exprL exprR node
replaceNE (CBinary CLeqOp exprL exprR node) = 
    CBinary CEqOp exprL exprR node
replaceNE (CBinary CGeqOp exprL exprR node) = 
    CBinary CEqOp exprL exprR node
replaceNE c = c
