...
translateStmt (CIf condExpr _ _ _) = translateExpr condExpr
{-
...
translateExpr (CAssign _ left right _) =
  App (I (N "=") []) Nothing [translateExpr left, translateExpr right]
...
translateExpr (CConst (CFloatConst (CFloat sf) _)) = fp2Ration . read $ sf
translateExpr (CConst (CIntConst (CInteger n _ _) _)) = fp2Ration . fromIntegral $ n
...
fp2Ration :: Float -> Expr
fp2Ration f = Lit (LitFrac (toRational f))
...
floatDecl name = CmdDeclareFun (N name) [] (TVar (N "Real"))
